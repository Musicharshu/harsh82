...
import config
from InsaneMusic import Carbon, YouTube, app
...
from InsaneMusic.utils.inline.playlist import close_markup
from InsaneMusic.utils.pastebin import Insanebin
from InsaneMusic.utils.stream.queue import put_queue, put_queue_index
...
async def stream(
    ...
    if forceplay:
        await Insane.force_stop_stream(chat_id)
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Insane.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path if direct else f"vid_{vidid}",
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Insane.join_call(chat_id, original_chat_id, file_path, video=None)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
